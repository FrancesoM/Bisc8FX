...
typedef struct {
    ...
    XDigital_setup_uio_map maps[ MAX_UIO_MAPS ];
} XDigital_setup_uio_info;

/***************** Variable Definitions **************************************/
static XDigital_setup_uio_info uio_info;
...
static int uio_info_read_name(XDigital_setup_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDigital_setup_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDigital_setup_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDigital_setup_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDigital_setup_Initialize(XDigital_setup *InstancePtr, const char* InstanceName) {
	XDigital_setup_uio_info *InfoPtr = &uio_info;
